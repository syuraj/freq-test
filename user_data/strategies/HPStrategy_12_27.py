...
class HPStrategy_12_27(IStrategy):
    INTERFACE_VERSION = 3
    max_safety_orders = 3
    lowest_prices = {}
    highest_prices = {}
    ...
    minimal_roi = {'0': 0.15, '30': 0.1, '60': 0.05, '90': 0.03, '120': 0.01, '240': 0}
    ...
    def informative_pairs(self):
        ...
        informative_pairs.extend(((btc_info_pair, self.timeframe), (btc_info_pair, self.inf_1h)))
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['fractal_top'] = (dataframe['high'] > dataframe['high'].shift(2)) & (dataframe['high'] > dataframe['high'].shift(1)) & (dataframe['high'] > dataframe['high']) & (dataframe['high'] > dataframe['high'].shift(-1))
        dataframe['fractal_bottom'] = (dataframe['low'] < dataframe['low'].shift(2)) & (dataframe['low'] < dataframe['low'].shift(1)) & (dataframe['low'] < dataframe['low']) & (dataframe['low'] < dataframe['low'].shift(-1))
        ...
        dataframe['volatility_factor'] = (dataframe['volatility'] - min_volatility) / (max_volatility - min_volatility)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dont_buy_conditions = [dataframe['pnd_volume_warn'] < 0.0, dataframe['btc_rsi_8_1h'] < 35.0]
        if conditions:
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float, time_in_force: str, exit_reason: str, current_time: datetime, **kwargs) -> bool:
        exit_reason = f'{exit_reason}_' + trade.buy_tag
...
class HPStrategyDCA(HPStrategy):
    ...
    def check_buy_conditions(self, lambo2_ema_14_factor, lambo2_rsi_4_limit, lambo2_rsi_14_limit, base_nb_candles_buy, low_offset, ewo_high, rsi_buy, base_nb_candles_sell, high_offset, ewo_low, buy_ema_cofi, buy_fastk, buy_fastd, buy_adx, buy_ewo_high, last_candle, previous_candle):
        lambo2 = (last_candle['close'] < last_candle['ema_14'] * lambo2_ema_14_factor.value) & (last_candle['rsi_4'] < int(lambo2_rsi_4_limit.value)) & (last_candle['rsi_14'] < int(lambo2_rsi_14_limit.value))
        conditions = [lambo2]
        buy1ewo = (last_candle['rsi_fast'] < 35) & (last_candle['close'] < last_candle[f'ma_buy_{base_nb_candles_buy.value}'] * low_offset.value) & (last_candle['EWO'] > ewo_high.value) & (last_candle['rsi'] < rsi_buy.value) & (last_candle['volume'] > 0) & (last_candle['close'] < last_candle[f'ma_sell_{base_nb_candles_sell.value}'] * high_offset.value)
        conditions.append(buy1ewo)
        buy2ewo = (last_candle['rsi_fast'] < 35) & (last_candle['close'] < last_candle[f'ma_buy_{base_nb_candles_buy.value}'] * low_offset.value) & (last_candle['EWO'] < ewo_low.value) & (last_candle['volume'] > 0) & (last_candle['close'] < last_candle[f'ma_sell_{base_nb_candles_sell.value}'] * high_offset.value)
        conditions.append(buy2ewo)
        crossed_above_fastk_fastd = previous_candle['fastk'] < previous_candle['fastd'] and last_candle['fastk'] > last_candle['fastd']
        is_cofi = (last_candle['open'] < last_candle['ema_8'] * buy_ema_cofi.value) & crossed_above_fastk_fastd & (last_candle['fastk'] < buy_fastk.value) & (last_candle['fastd'] < buy_fastd.value) & (last_candle['adx'] > buy_adx.value) & (last_candle['EWO'] > buy_ewo_high.value)
        conditions.append(is_cofi)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float, min_stake: float, max_stake: float, **kwargs):
        ...
        if dt.is_monotonic_increasing:
            logging.info(f'Percentage drop se pro {trade.pair} stále zvětšuje, DCA se neprovádí.')
            return None
...
class HPStrategyTF(HPStrategyDCA):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        down_trend = dataframe['higher_tf_trend'] > 1
...
class HPStrategyTFJPA(HPStrategyTF):
    ...
    def populate_buy_trend_sr(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if metadata['pair'] in self.support_dict and metadata['pair'] in self.resistance_dict:
            ...
            if supports and resistances:
                ...
                dataframe['nearest_resistance'] = dataframe['close'].apply(lambda x: min([resistance for resistance in resistances if resistance >= x], default=x, key=lambda resistance: abs(x - resistance)))
                # Vypočítání procentního rozdílu mezi cenou a nejbližším supportem/resistencí
                dataframe['distance_to_support_pct'] = (dataframe['nearest_support'] - dataframe['close']) / dataframe['close'] * 100
                dataframe['distance_to_resistance_pct'] = (dataframe['nearest_resistance'] - dataframe['close']) / dataframe['close'] * 100
                # Vygenerování nákupních signálů na základě supportu a resistence
                buy_threshold = 0.1  # 0.1 %
                dataframe.loc[(dataframe['distance_to_support_pct'] >= 0) & (dataframe['distance_to_support_pct'] <= buy_threshold) & (dataframe['distance_to_resistance_pct'] >= buy_threshold), 'buy_signal'] = 1
                dataframe.loc[(dataframe['distance_to_support_pct'] >= 0) & (dataframe['distance_to_support_pct'] <= buy_threshold) & (dataframe['distance_to_resistance_pct'] >= buy_threshold), 'enter_tag'] += 'sr_buy_mid'
                # Odebrání pomocných sloupců
                dataframe.drop(['nearest_support', 'nearest_resistance', 'distance_to_support_pct', 'distance_to_resistance_pct'], axis=1, inplace=True)
        # Přidání podmínek pro EMA a objem
        dataframe.loc[(dataframe['volume'] > 0) & (dataframe['ema_diff_buy_signal'].astype(int) > 0), 'buy_ema'] = 1
        dataframe.loc[(dataframe['volume'] > 0) & (dataframe['ema_diff_buy_signal'].astype(int) > 0), 'enter_tag'] += 'ema_dbs_'
        # Generování nákupních signálů pouze pokud jsou splněny obě podmínky
        dataframe.loc[(dataframe['buy_signal'] == 1) & (dataframe['buy_ema'] == 1) & (dataframe['rsi'] <= dataframe['weighted_rsi']), 'enter_long'] = 1
        # Odebrání pomocných sloupců
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if metadata['pair'] in self.support_dict:
            ...
            if s:
                ...
                dataframe['nearest_support'] = dataframe['close'].apply(lambda x: min([support for support in s if support <= x], default=x, key=lambda support: abs(x - support)))
                if 'nearest_support' in dataframe.columns:
                    ...
                    buy_threshold = 0.1  # 0.1 %
                    dataframe.loc[(dataframe['distance_to_support_pct'] >= 0) & (dataframe['distance_to_support_pct'] <= buy_threshold), 'buy_support'] = 1
                    dataframe.loc[(dataframe['distance_to_support_pct'] >= 0) & (dataframe['distance_to_support_pct'] <= buy_threshold), 'enter_tag'] += 'sr_buy'
                    # Odebrání pomocných sloupců
                    dataframe.drop(['nearest_support', 'distance_to_support_pct'], axis=1, inplace=True)
        # Přidání podmínek pro EMA a objem
        dataframe.loc[(dataframe['volume'] > 0) & (dataframe['ema_diff_buy_signal'].astype(int) > 0), 'buy_ema'] = 1
        dataframe.loc[(dataframe['volume'] > 0) & (dataframe['ema_diff_buy_signal'].astype(int) > 0), 'enter_tag'] += 'ema_dbs_'
        # Generování nákupních signálů pouze pokud jsou splněny obě podmínky
        dataframe.loc[(dataframe['buy_support'] == 1) & (dataframe['buy_ema'] == 1) & (dataframe['rsi'] <= dataframe['weighted_rsi']), 'enter_long'] = 1
        # Odebrání pomocných sloupců
        if 'buy_support' in dataframe.columns:
            dataframe.drop(['buy_support'], axis=1, inplace=True)
        if 'buy_ema' in dataframe.columns:
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float, min_stake: float, max_stake: float, **kwargs):
        ...
        if trade.pair in self.support_dict:
            ...
            df['nearest_support'] = df['close'].apply(lambda x: min([support for support in s if support <= x], default=x, key=lambda support: abs(x - support)))
            if 'nearest_support' in df.columns:
                ...
                if 'nearest_support' in last_candle:
                    ...
                    distance_to_support_pct = abs((nearest_support - current_rate) / current_rate)  # Datový typ: float, jednotka: %
                    ...
                    if 0 <= distance_to_support_pct <= 0.01 or current_rate < nearest_support:
                        ...
                        count_of_buys = sum((order.ft_order_side == 'entry' and order.status == 'closed' for order in trade.orders))  # Datový typ: int
                        # Zjištění času posledního nákupu
                        last_buy_time = max([order.order_date for order in trade.orders if order.ft_order_side == 'entry'], default=trade.open_date_utc)
                        ...
                        if self.max_safety_orders >= count_of_buys:
                            ...
                            for order in reversed(trade.orders):
                                if order.ft_order_side == 'entry' and order.status == 'closed':
...
class HPStrategyJSTKR(HPStrategyTFJPA):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe = super().populate_buy_trend(dataframe, metadata)
        dataframe.loc[dataframe['jstkr_3'] == 1, 'enter_long'] = 1
        dataframe.loc[dataframe['jstkr_3'] == 1, 'enter_tag'] += 'jstkr_3_'
        return dataframe
